...
**Original-icon extraction rule**:

- When image usage includes **B) User-provided** and the project images contain diagrams, logos, screenshots, or architecture visuals with reusable embedded icons, run icon detection after image analysis:
  ```bash
  python3 scripts/detect_icons.py --project-path <project_path>
  ```
- Use the generated `icon_detection/summary.json` and each image's `*.usable_icons.json` to identify candidate reusable assets.
- These extracted assets are **candidate original-icon resources**, not guaranteed final assets. Strategist must record them for Executor evaluation, not promise that every extracted crop will be used.
- Built-in library selection and extracted original-icon assets are separate concepts:
  - Built-in library choice still defines the default icon system for newly introduced or replacement icons.
  - Extracted original-icon assets are only for fidelity-critical reuse cases where source-specific visuals matter more than replacing them with generic library icons.
...
**When selection includes B and the image set likely contains reusable embedded icons**, run icon detection immediately after image analysis:
...
- keep the normal image inventory in **section VIII. Image Resource List**,
- add extracted icon candidates to **section VI. Icon Usage Specification** under an extracted-assets subsection,
- treat the extracted icons as optional reusable assets for Executor, with confidence and fallback notes.
...
> **Design spec handoff rule for extracted icons**: If `detect_icons.py` produced usable icon assets, Strategist must include them in **section VI** as an explicit candidate asset inventory. Do **not** merge these rows into section VIII unless the full source image itself is also planned for use on slides.
