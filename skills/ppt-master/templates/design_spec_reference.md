...
### Extracted Original Icon Assets (if available)

> Use this subsection only when `icon_detection/summary.json` and `*.usable_icons.json` exist for the project. These are **candidate reusable assets** from user-provided source images. They do not replace the built-in icon library decision above; they supplement it for fidelity-critical reuse cases.

| Source Image | Extracted Asset | Label | Confidence Band | Suggested Use | Candidate Slides | Fallback |
| ------------ | --------------- | ----- | --------------- | ------------- | ---------------- | -------- |
| [e.g. images/system_arch.png] | `icon_detection/crops/system_arch/icon_001.png` | database icon | High | Reuse source-specific architecture node icon | Slide 04, Slide 05 | Use built-in database icon from locked library |

**Recording rules**:

- Only include assets from `*.usable_icons.json`, not from raw detections.
- Treat every row as a **candidate asset**, not a guaranteed final asset.
- Add a fallback recommendation for each row so Executor can choose between crop reuse, built-in library icon, or redraw.
- If no extracted icon assets are relevant, omit this subsection entirely.
...
> **Boundary rule**: This section tracks full image assets used or prepared for slides. Do **not** list extracted icon crops here unless the full source image is also independently planned as a slide image asset. Extracted icon candidates belong in **section VI. Icon Usage Specification**.
