...
### 6.1 Extracted Original Icon Assets

If the Design Specification's **VI. Icon Usage Specification** includes an **Extracted Original Icon Assets** subsection, Executor must treat those rows as a separate candidate-asset inventory.

These assets are not generic icon-library placeholders. They are reusable crops extracted from user-provided source images and should be evaluated with a fidelity-first mindset.

**Decision order**:

1. **Use extracted original icon crop** when the source-specific visual identity matters and the crop will remain clear at slide scale.
2. **Use built-in icon library icon** when the extracted crop is low quality, visually noisy, too small, or unnecessary because a generic replacement is acceptable.
3. **Redraw natively with SVG shapes** when the element is geometric, simple, and benefits more from editability and style consistency than from literal source reuse.
4. **Keep a larger source image region** only when the icon cannot be meaningfully separated from its surrounding diagram context.

**Prefer extracted original icon crops when**:

- the icon is branded, proprietary, or source-specific,
- the icon style is central to the source diagram's meaning,
- preserving visual fidelity is more important than recolorability,
- the crop remains legible after final placement size is applied.

**Prefer built-in icon library icons when**:

- the extracted crop is blurry, noisy, aliased, or poorly bounded,
- the icon is semantically generic and a library substitute is acceptable,
- a sharper, recolorable, or more stylistically unified icon is needed,
- the Design Spec fallback recommendation points to a locked library replacement.

**Prefer redraw when**:

- the visual element is mostly composed of simple geometric primitives,
- the crop introduces raster artifacts that reduce deck quality,
- cross-page consistency matters more than literal reuse,
- the same visual motif will repeat across multiple slides and should stay editable.

**Do not assume extracted assets must be used**. Every extracted-icon row is a candidate asset, not a mandatory asset.

### 6.2 Executor Constraints For Extracted Icon Assets

- Only use assets listed in the Design Spec's extracted-icon subsection. Do not browse `icon_detection/` arbitrarily.
- Read the row's fallback recommendation before deciding to reuse the crop.
- When a crop is used, reference it as an image asset; do not convert it into icon placeholder syntax.
- If crop quality is insufficient, fall back to the approved built-in library or redraw instead of forcing the extracted image into the page.
- Keep usage selective. Overusing raster icon crops weakens editability and can make the deck feel inconsistent.
